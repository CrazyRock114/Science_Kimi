...
## 七、进展快照与待办方向（2026-07-30 更新）

### 已完成（超出原计划范围）
- Phase 0–4 全部完成：三科内容模型、化学虚拟实验室（102 实验）、物理/生物仿真、3D 细胞工坊、AI 后端与导师面板、TTS 讲解系统。
- **IGCSE_miniMax 内容融合**：944 条考纲 statement 全量导入；75 节课转换入库（412 道考试真题带 mark scheme）；21 个仿真基元层移植；生物 18 个解剖交互模块；三科考纲覆盖率 100%。
- **性能**：全部 145 个知识点路由级懒加载，主 chunk 2.77MB → 404KB；vendor 分包（react/katex/i18n）。
- **内容保真**：narration 行级剧本 + 357 个仿真联动 action（音画同步）、99 个公式实时数值代入全部恢复。
- **考纲地图**：`/:lang/syllabus/:code` 页面，statement 级覆盖可视化（Core/Supplement 分档），首页与学科页入口。
- 测试 5000 个全绿；GitHub Pages 自动部署流水线。

### 待办方向（上线后再决定优先级）
1. **SEO 与无障碍**：静态预渲染、meta/OG、reduced-motion 完善、键盘导航。
2. **AI 实测与调优**：需 LLM API key（`.env`，参照 `.env.example`）实测导师问答与学习报告质量；后端部署到云函数。
3. **TTS 音频预生成**：按 `tools/generate-narration-audio.md` 跑 edge-tts（注意现为行级剧本，音频按行 id 产出）。
4. **人教版线内容扩充**：自研知识点 70 个 vs IGCSE 线 75 课，国内教材线深度待补齐（可参照 IGCSE 课的结构升级：学习目标、词汇表、真题演练）。
5. **题库扩充**：AI 生产管线（AI 起草 → 内核校验 → 人工审校）。
6. **Paper 5/6 实验技能模块**：IGCSE 实验考试专项（占分 20%），miniMax 侧也未做，是差异化空间。
7. **国内正式上线**：域名 + ICP 备案（周期长，尽早启动）→ 腾讯云 COS + CDN；AI 后端上云函数。
8. **小优化项**：手写课路由的 Loading 闪烁（可预取）、vendor-katex 258KB 首屏同步加载（可改懒加载）、`npm run extract:metas` 手动步骤（可加 pretest 钩子）、`igcse-statements.ts` 少量 zh 以英文填充的条目待翻译。
